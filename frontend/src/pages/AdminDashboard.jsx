  ...
  if (!authLoading && (!isAuthenticated || profile?.role !== "admin")) {
    return (
      <div className="flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center bg-slate-950 px-6 text-center text-white">
        ...
        <p className="mt-2 text-slate-400">You must be logged in as an Admin to access this page.</p>
        <Link
          to="/"
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-slate-800 bg-slate-900/80 text-xs font-semibold uppercase tracking-wider text-slate-400">
                <tr>
                  ...
                  <th className="px-6 py-4">Solved / Submissions</th>
                  ...
                  <tr key={user.user_id} className="hover:bg-slate-800/40 transition">
                    <td className="px-6 py-4 font-medium">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-white">{user.display_name || user.username}</p>
                    ...
                    <td className="px-6 py-4 text-slate-400">
                      <span className="text-green-400 font-semibold">{user.problems_solved || 0}</span> / {user.total_submissions || 0}
                    </td>
...
function ProblemsManagement() {
  ...
  const openEditModal = async (prob) => {
    ...
    try {
      ...
      if (fullData && fullData.test_cases && fullData.test_cases.length > 0) {
        setFormData((prev) => ({
          ...prev,
          test_cases: fullData.test_cases
        }));
      }
    } catch (err) {
  ...
  const addTestCase = () => {
    setFormData((prev) => ({
      ...
      test_cases: [
        ...prev.test_cases,
        { input: "", expected_output: "", is_sample: prev.test_cases.length === 0, explanation: "" }
      ]
    }));
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editingProblem) {
        await adminService.updateProblem(editingProblem.problem_id, formData);
        setActionMessage("Problem updated successfully");
      } else {
        const slug = formData.slug || formData.title.toLowerCase().replace(/[^a-z0-9]+/g, "-");
        await adminService.createProblem({ ...formData, slug });
        setActionMessage("Problem created successfully");
      }
      setModalOpen(false);
      setTimeout(() => setActionMessage(null), 3000);
      fetchProblems();
  ...
  const handleDeleteProblem = async (problemId, title) => {
    ...
    try {
      ...
      setActionMessage(`Problem "${title}" deleted`);
      setTimeout(() => setActionMessage(null), 3000);
      fetchProblems();
    } catch (err) {
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">Create, edit, and organize coding problems.</p>
        </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filteredProblems.map((prob) => (
                  <tr key={prob.problem_id} className="hover:bg-slate-800/40 transition">
                    <td className="px-6 py-4 font-mono text-xs text-slate-500">#{prob.problem_id}</td>

                    <td className="px-6 py-4 font-medium">
                      <Link to={`/problems/${prob.problem_id}`} className="hover:text-indigo-400 transition">
                        {prob.title}
                      </Link>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => handleDeleteProblem(prob.problem_id, prob.title)}
                          className="rounded-lg border border-red-500/30 bg-red-500/10 p-1.5 text-red-400 hover:bg-red-500/20 transition"
          ...
          >
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col min-h-0 flex-1">
              <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Input Format</label>
                    <textarea
                      rows={2}
                  ...
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Output Format</label>
                    <textarea
                      rows={2}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Time Limit (seconds)</label>
                    <input
                      type="number"
                      step="0.1"
                      min="0.1"
                      ...
                      onChange={(e) => setFormData({ ...formData, time_limit: parseFloat(e.target.value) || 1.0 })}
                      className="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-white outline-none focus:border-indigo-500"
                      placeholder="1.0"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Memory Limit (MB)</label>
                    <input
                      type="number"
                      step="1"
                      min="16"
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Constraints</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 rounded-xl border border-slate-800 bg-slate-950 p-3 max-h-40 overflow-y-auto">
                    {allTags.length === 0 ? (
                      <p className="text-xs text-slate-500 py-1">Loading tags...</p>
                    ) : (
                      allTags.map((tag) => {
                          ...
                          <button
                            ...
                            onClick={() => {
                              const newTags = isSelected
                                ? formData.tags.filter((id) => id !== tag.tag_id)
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <label className="block text-xs font-semibold uppercase text-slate-400">
                      Sample & Judge Test Cases ({formData.test_cases?.length || 0})
                    </label>
                    <button
                      type="button"
                      onClick={addTestCase}
                      ...
                      >
                        <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
                          <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-slate-300">
                            ...
                            />
                            <span>Sample Test Case {tc.is_sample ? "(Visible to users)" : "(Hidden Judge)"}</span>
                          </label>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          <div>
                            <span className="block text-[11px] font-semibold text-slate-400 mb-1">Standard Input</span>
                            <textarea
                              rows={2}
                              value={tc.input}
                          ...
                          <div>
                            <span className="block text-[11px] font-semibold text-slate-400 mb-1">Expected Output</span>
                            <textarea
                              rows={2}
                        ...
                        {tc.is_sample && (
                          <div>
                            <span className="block text-[11px] font-semibold text-slate-400 mb-1">Explanation (Optional)</span>
                            <input
                              type="text"
